 ...
 * If no node is available given the threshold, it will perfom a super attack on the node with the greater SV if possible
 * If not, it will perform a normal attack in the node that has the smallest SV.
 ...
 * @author Alex Vargas
 */
public class DeadHand extends Attacker {

    private final static String attackerName = "DeadHand";
     ...
     */
	public DeadHand(String defenderName, String graphFile) {
	...
	public AttackerAction makeAction() {
		...
		if(mNodeToAttack == null){
			if(Parameters.SUPERATTACK_RATE < budget){ //Perform superattack on strongest node
				type = AttackerActionType.SUPERATTACK;
				for(Node mNode : availableNodes){
					if(mNodeToAttack == null || mNodeToAttack.getSv() < mNode.getSv())
						mNodeToAttack = mNode;
				}
			}else{ //Perform normal attack on weakest node
				type = AttackerActionType.ATTACK;
				for(Node mNode : availableNodes){
					if(mNodeToAttack == null || mNodeToAttack.getSv() > mNode.getSv())
						mNodeToAttack = mNode;
